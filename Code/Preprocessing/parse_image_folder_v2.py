...
def parse_folder_structure(root_path):
    ...
    for sub_folder in os.listdir(root_path):
        ...
        for patient_folder in os.listdir(os.path.join(root_path, sub_folder)):
            if not patient_folder.startswith("Patient_"):
                continue
            patient_path = os.path.join(root_path, sub_folder, patient_folder)
...
# Usage
root_path = r"P:\Dataset\MO-DBT"
df_folders = parse_folder_structure(root_path)
df_folders.to_excel("P:/Dataset/R01-MO-DBT/MO-DBT-data-curation/R3Data/dicomtocsv_series_upto_{date}.xlsx", index=False)